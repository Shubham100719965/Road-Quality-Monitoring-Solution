...
2. Run the launcher:
   ...
   ```bat
   run_detection.bat image --nogui
   ```

   For video processing:

   ```bat
   run_detection.bat video --nogui --input data/input/traffic_signs.mp4 --max-frames 10
   ```

   The scripts will create a local virtual environment if needed, install dependencies, and save annotated outputs into the runs/detect/predict folder.
...
4. Run the scripts
   - To process images
        ```bash
        python process_image.py --nogui
        ```

    - To process videos
        ```bash
        python process_video.py --nogui
        ```
## Limitations
